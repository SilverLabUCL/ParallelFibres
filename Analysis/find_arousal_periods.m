...
function [aroused,quiescent] = find_active_periods(MI_time,MI_smooth,varargin)
    ...
    MI_baseline = median(MI_smooth(:));
    MI_std = std(MI_smooth(:));
    start_speed_threshold = .01;
    end_speed_threshold = .005;
    end_speed_time = 150; %ms
    end_speed_bins = round(end_speed_time/mean(mean(diff(MI_time))));
    ...
    while isDone == 0
        ...
        if numel(whisk_start) == 0
            ...
            if numel(slowing_start) == 0 
                whisk_end = size(MI_time,1);
                if max(x(whisk_start:whisk_end))>MI_threshold
                    aroused = [aroused; MI_time(whisk_start), MI_time(whisk_end)];
                end
                isDone = 1;
            else
                whisk_start = whisk_start(1);
    ...
    quiescent = [];
    if numel(aroused)==0
        quiescent = [MI_time(1),MI_time(end)];
    else            
        if aroused(1,1) > MI_time(1)
            quiescent = [quiescent; MI_time(1), aroused(1,1)];
        end
        for k = 1:size(aroused,1)-1
            quiescent = [quiescent; aroused(k,2), aroused(k+1,1)];
        end 
        if aroused(end,2) < MI_time(end)
            quiescent = [quiescent; aroused(end,2), MI_time(end)];
        end
    end


%% test
%figure, plot(MI_time,MI_smooth,'k')
